...
const InsightsPanel: React.FC<{ insights: string[] }> = ({ insights }) => {
  ...
  const insightTimeoutsRef = useRef<NodeJS.Timeout[]>([]);
  
  useEffect(() => {
    // Clear any existing timeouts
    insightTimeoutsRef.current.forEach((timeout: NodeJS.Timeout) => clearTimeout(timeout));
    ...
    insights.forEach((insight, index) => {
      ...
      insightTimeoutsRef.current.push(timeout);
    });
    
    // Cleanup on unmount or when insights change
    return () => {
      insightTimeoutsRef.current.forEach((timeout: NodeJS.Timeout) => clearTimeout(timeout));
      insightTimeoutsRef.current = [];
    };
...
const ResultsPageV2: React.FC<ResultsPageV2Props> = ({ results, onBack, isDarkMode }) => {
  ...
  const showActionsTimeoutRef = useRef<NodeJS.Timeout | null>(null);
  
  useEffect(() => {
    showActionsTimeoutRef.current = setTimeout(() => setShowActions(true), 2000);
