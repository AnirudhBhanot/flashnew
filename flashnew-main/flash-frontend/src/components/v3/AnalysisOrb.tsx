...
export const AnalysisOrb: React.FC<AnalysisOrbProps> = ({ 
  ...
  useAnimationFrame((time, delta) => {
    // Only update canvas when actively analyzing
...
export const MinimalResults: React.FC<{ data: any }> = ({ data }) => {
  ...
  const getScoreColor = (score: number) => {
    ...
    if (score >= colors.excellent.min) return '#FFFFFF'; // White for excellent
    if (score >= colors.good.min) return '#E8EAED'; // Light gray for good
    if (score >= colors.fair.min) return '#9CA3AF'; // Medium gray for fair
    return '#6B7280'; // Dark gray for poor
  };
